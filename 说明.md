...
│   ④ HTML5 Audio 播放     │                        │   RTX 4070 Super 加速   │
└──────────────────────────┘                        └─────────────────────────┘
```
...
| 🎭 54 种声音 | 内置美式男声、女声、英式声音，无需自备参考音频 |
...
|------|------|---------|
| 操作系统 | Windows 10/11 | ✅ |
| 显卡 | NVIDIA GPU（可选，CPU 也可运行） | ✅ RTX 4070 Super 16GB |
...
在 `server.py` 顶部修改 `VOICE` 变量来切换声音：
...
> 💡 桌面上已创建 **Kokoro TTS** 快捷方式，双击即可启动托盘程序。
